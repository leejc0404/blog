# KoreaPlug 자동 이미지 삽입 태스크 (v5.9 — 다중 글 연속 처리 + zoom 뷰포트 보호 + 세션 사전 점검 + 사건 설계 + 한국성 + 신체 결손 방지 + 반응형 전송 + Google Flow)

> 🔭 **v5.9 변경 (2026-08-31) — `computer: zoom` 이 뷰포트를 축소시킨 채 복구되지 않아, 후보 절반을 보지 못한 상태로 채택했다.**
> 2026-08-31 실행에서 5-3 판정 중 `computer: zoom` 을 호출하자 Flow 탭의 뷰포트가 **1920×919 → 807×227로 고착**됐다. `window.outerWidth` 는 1920 그대로였고 `innerWidth` 만 807이었다 — CDP 디바이스 에뮬레이션이 걸린 뒤 해제되지 않은 것이다. 증상은 세 가지로 나타났다.
> - ① **그리드 lazy load가 6장에서 멈췄다.** 뷰포트가 좁아 7·8번째 카드가 관측 영역 밖으로 밀렸고, `scrollIntoView`·`scrollTop` 조작·`computer: scroll` 어느 것으로도 추가 로드되지 않았다. **히어로 후보 2장 중 뒤쪽을 끝내 볼 수 없어**, 앞쪽 1장으로 확정할 수밖에 없었다.
> - ② **`resize_window` 로 복구되지 않았다.** 창 크기는 바뀌지만 에뮬레이션된 `innerWidth` 는 그대로다.
> - ③ **이후 zoom 호출이 연쇄 실패했다.** 에러 메시지가 `Region exceeds viewport boundaries (807x227)` 처럼 **직전 zoom 결과 이미지의 크기**를 뷰포트로 보고해, 어떤 좌표를 넣어도 통과하지 못했다. `Page.captureScreenshot timed out` 도 같은 구간에서 3회 발생했다.
> → v5.9는 ① **5-3 판정을 `scale 0.5` 스크린샷 1장으로 끝내는 것을 원칙으로 못 박고**, zoom은 해부 검사처럼 꼭 필요할 때만 **캡처 직전 1회**로 제한한다 ② **뷰포트 고착 감지·복구 절차**를 신설한다(캡처 전이면 프로젝트 URL 재이동이 유일한 해법, 캡처 후면 재이동 금지) ③ 5-4 캡처 직전 **`innerWidth` 확인**을 필수 항목으로 넣는다.
>
> 🔁 **함께 신설 (2026-08-31) — 하루에 2건 이상을 처리하는 절차가 아예 없었다.**
> 같은 날 대상 글이 **2건(#148·#149)** 이었는데, 舊 지침서는 "대상이 여러 건이면 오래된 날짜부터 각각 처리한다"(STEP 1) 한 줄뿐이고 **STEP 5~7을 두 번 도는 방법은 어디에도 없었다.** 실행 중 두 가지가 실제로 깨졌다.
> - **`window._wpWin` 소실**: 2번째 글을 위해 Flow 탭을 재이동하면 `window._wpWin` 이 사라지는데, STEP 6-1은 이를 "최초 1회"로만 기술하고 있어 `Cannot read properties of undefined (reading 'postMessage')` 로 업로드가 막혔다.
> - **`window._uploadedIds` 오염**: 1번째 글의 업로드 결과가 배열에 남아 있어, 초기화하지 않으면 2번째 글의 삽입이 1번째 글 이미지를 집는다.
> → **STEP 7.9 「다중 글 연속 처리」** 를 신설해 글 단위 라운드 경계와 초기화 대상을 명시한다.
...
> ⛔ **비밀번호·2단계 인증 코드 입력은 어떤 경우에도 루틴이 대신하지 않는다.** ※ 이 항목은 **v5.6(2026-08-26)에서 완화됐다** — 이미 뜬 계정 선택 화면에서 `leejc0404@gmail.com` 클릭과 이어지는 OAuth 동의 「계속」은 사용자 사전 승인으로 **허용**된다. 최신 기준은 **STEP 0 「공통 원칙 — 로그인 취급」 표**를 따른다.
...
- 모드: **이미지** (동영상 아님)
- 비율: **16:9**
- 장수: **x2** — 2026-08-26 실측 기본값은 **x1** 이었다. 칩이 `x1 x2 x3 x4` 로 붙어 있어 **x3를 잘못 누르기 쉬우니** 저장 전 재확인
...
확인 후 입력바에 **`Nano Banana 2 ▭ x2`** 가 표시되면 정상이다. 이 표시가 5-2 제출 전 마지막 관문이다.
...
browser_batch([
  ...
  {computer: wait 4},
  {computer: screenshot scale 0.35}      ← 반드시 배치의 마지막
])
```

🚨 **(v5.6) 첫 프롬프트는 반드시 포커스를 검증하고 넣는다.** 하단 입력창을 노렸다고 생각한 클릭이 **상단 검색창에 들어가** 프롬프트가 통째로 유실되고 필터까지 걸리는 사고가 두 사이트에서 반복됐다 (0and1Life 2026-08-25 · KoreaPlug 2026-08-26). 첫 클릭 직후 아래 한 줄을 `javascript_tool` 로 실행하면 1회 호출로 막을 수 있다.
...
- `ph` 에 **"무엇을 만들고 싶으신가요"** 계열 문구가 잡히고 `top` 이 뷰포트 높이의 80% 이상이면 정상이다
- 상단 검색창(`top` 이 100 미만)이 잡혔다면 **타이핑하지 말고** ⓐ 검색창 `✕` 로 닫고 ⓑ 필터 칩(`동영상 ✕`·`업로드됨 ✕`)을 해제한 뒤 ⓒ 다시 하단 입력창을 클릭한다
...
⚠️ **(v5.6) 전송 직후 같은 좌표를 재클릭하지 않는다.**

전송하면 입력창이 접히면서 레이아웃이 올라가, **직전 입력창 좌표에 상단 검색창이 온다.** 그 상태로 타이핑하면 프롬프트가 검색어로 들어간다.

> 🚨 **2026-08-26 실측**: 전송 4초 뒤 같은 좌표 `(700, 622)` 를 클릭했더니 상단 검색창이 잡혔고, 1,300자 프롬프트가 검색어로 입력되면서 `동영상 ✕`·`업로드됨 ✕` 필터가 걸려 **그리드가 통째로 비었다.** 필터 해제·복구에 5회를 더 썼다.

배치는 반드시 아래 순서로 짠다. **한 배치 안에서 클릭 → 타이핑 → 전송을 끝내고, 다음 프롬프트는 새 배치로 시작한다.**

```
browser_batch([
  {computer: left_click → 입력창},
  {computer: type       → 프롬프트 전문},
  {computer: wait 1},
  {computer: left_click → 전송 화살표},
  {computer: wait 4},
  {computer: screenshot scale 0.35}      ← 배치의 마지막
])
```

⛔ **전송 뒤에 클릭·타이핑이 오는 배치를 만들지 않는다.** 전송 이후 좌표는 전부 무효로 간주한다.
ℹ️ 필터 칩이 걸려 그리드가 비었다면, 칩의 `✕` 를 누르거나 프로젝트 URL로 재이동해 초기화한다. **`window._img*` 캡처 전이라면 재이동해도 손실이 없다** — 캡처 후에는 절대 재이동 금지(5-4 참조).

#### 5-3. 채택본 선택 (v4.2 — 그리드 스크린샷 1장으로 판정)

⛔ **에디터 뷰에 들어가지 않는다.** 후보를 하나씩 클릭해 여는 것이 舊 실행에서 20회 왕복을 잡아먹은 원인이다.
5-2 마지막 스크린샷 **한 장에 모든 후보가 나란히 보이므로**, 그 화면에서 바로 판정한다.

🚨 **(v5.9) 판정은 `scale 0.5` 스크린샷 1장으로 끝내는 것이 원칙이다. `computer: zoom` 을 습관적으로 쓰지 않는다.**
...
- 세부가 필요하면 먼저 **`scale 0.5` 로 다시 찍는다.** 0.3~0.35로 찍었다면 그것만으로 판정이 안 되는 게 당연하다.
...
- 🩻 **(v5.3) 해부 검사 — 인물이 있는 후보는 이것을 먼저 본다.** 머리·목·팔·다리 중 **없는 부위가 있는가?** 옷만 있고 그 안이 비어 있는가? 손가락이 6개인가? **하나라도 해당하면 즉시 탈락**이며, 다른 기준이 아무리 좋아도 채택하지 않는다. 후보 2장이 모두 걸리면 3-3 ③의 「몸통이 보이는 인물」 형식으로 수정 재생성한다. 그리드 썸네일에서 인물이 작아 판별이 어려우면 **`scale 0.5` 스크린샷을 먼저 다시 찍고**, 그래도 판별이 안 될 때만 zoom을 쓴다. **(v5.9) zoom은 인물이 있는 카드들을 한 번에 담는 넓은 영역으로 1회만** 부르고, 호출 후 위의 뷰포트 고착 검사를 반드시 돌린다 — 이 항목은 확대 비용을 감수할 가치가 있지만, 고착되면 남은 후보를 못 보게 되므로 순서를 지킨다.
...
- 🎛 **(v5.6) Flow 설정 `에이전트 OFF / 이미지 / 16:9 / Nano Banana 2 / x2` 를 매 실행 눈으로 확인한다 (STEP 5-1).** '최초 1회'가 아니다 — 기본값이 실행마다 `x1`·`Nano Banana 2 Lite`·`동영상` 으로 돌아가 있었다. **에이전트 ON이면 프롬프트가 별도 채팅 세션으로 넘어가 재해석되고 생성이 전량 실패한다** (2026-08-26 실측: 히어로 2장 모두 `실패`, 세션 패널이 그리드를 덮어 인지도 늦었다). 설정 변경 후 **저장** 버튼 필수

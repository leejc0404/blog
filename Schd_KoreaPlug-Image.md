# KoreaPlug 자동 이미지 삽입 태스크 (v6.0 — Flow 교차출처 캡처 + DOM 기반 제출 + 다중 글 연속 처리 + zoom 뷰포트 보호 + 세션 사전 점검 + 사건 설계 + 한국성 + 신체 결손 방지 + 반응형 전송 + Google Flow)
...
**「전송·검증」 스니펫** — 타이핑 직후 실행. 전송 전후 편집기 길이를 함께 반환한다.

```javascript
await new Promise(r => setTimeout(r, 800));
const b  = Array.from(document.querySelectorAll('button')).find(b => (b.getAttribute('aria-label')||'') === '생성 시작');
const ed = document.querySelector('[contenteditable="true"], textarea');
const before = (ed.value || ed.textContent).length;
b.click();
await new Promise(r => setTimeout(r, 3000));
'before:' + before + ' after:' + (ed.value || ed.textContent).length
```

- `before` 가 프롬프트 길이(보통 1,100~1,700)이고 **`after` 가 14 전후**(플레이스홀더 "무엇을 만들고 싶으신가요?")면 전송 성공이다
- `after` 가 `before` 와 같으면 전송되지 않은 것이다. `b` 가 `undefined` 면 버튼 aria-label이 바뀐 것이므로 `Array.from(document.querySelectorAll('button')).map(b=>b.getAttribute('aria-label'))` 로 현재 라벨을 확인한다
- `before` 가 14면 타이핑이 들어가지 않은 것이다 — 포커스 스니펫부터 다시

...
> | # | 조치 | 근거 (2026-09-08 실측) |
> |---|---|---|
> | 1 | **`genCount` 에 `- stock` 추가** (STEP 2) | ev2·stk1·deco0 → 舊 3장(총 6) / 新 2장(총 5) |
> | 2 | **분류기 ⑧단계 신설** — WP 리사이즈본·비-webp 사진을 데코로 확정 | Post 721 음식 사진 5장이 전부 `UNKNOWN` → 수동 확인 결과 전부 데코였다 |
> | 3 | **Flow `x2` 자동 교정 JS** (STEP 5-1) | 설정이 또 `x1` 로 되돌아가 있었다 (3회 연속) |
> | 4 | **`저장` 버튼 조항 삭제** (STEP 5-1) | 현재 UI에 저장 버튼이 없다 (`save:none`) — 舊 지침이 오류였다 |
> | 5 | **스크린샷 타임아웃 시 JS 대체 기준** (STEP 5-3) | 설정 확인용 스크린샷 1회 타임아웃 → JS로 대체해 재촬영 없이 진행 |
>
> 동시에 **Draft 루틴**(증빙 캡처 `srcset` 누락)과 **Writer 루틴**(히어로 헤더 템플릿 부재)의 대응 조항도 같은 날 각 파일에 반영했다. 세 루틴을 함께 갱신해야 이 회차의 문제가 전부 닫힌다.
>
> ⚠️ **KoreaPlug 미반영**: 위 1·2·3·4·5는 `Schd_KoreaPlug-Image.md` 에도 동일하게 존재하는 결함이다. 사용자 지시로 이번 회차는 0and1Life만 수정했다. **다음 회차에 백포트 필요.**
...
window._cls = window._postData.map(p => {
  ...
  const wpSizeRe  = /-\d{2,4}x\d{2,4}\.(jpe?g|png|webp|gif)$/i;               // ⑧ WP 리사이즈본 파일명 (v6.2)
  const photoRe   = /\.(jpe?g|png|gif)$/i;                                    // ⑧ 비-webp 일반 사진 (v6.2)
  ...
  for (const img of Array.from(div.querySelectorAll('img'))) {
    ...
    else if (wpSizeRe.test(base) || photoRe.test(base))
                                   { k = 'deco';     why = 'legacy-photo'; }   // ⑧ (v6.2) 마커 없는 옛 사진 = 데코 확정
...
🆕 **(v6.2) ⑧단계 신설 — 마커 없는 옛 사진을 데코로 확정해 `unknown`을 줄인다.**
증빙 캡처는 이 루틴과 Draft 루틴 모두 **`.webp`** 로만 만든다. 반대로 **`-225x300.jpg` 같은 WP 리사이즈본 파일명이나 `.jpg`/`.png`/`.gif` 원본은 사람이 올린 사진**이고, 증빙이라면 ①②⑤⑥⑦ 중 하나에는 반드시 걸린다. 그러므로 여기까지 내려온 비-webp 파일은 **데코로 확정**해도 안전하다.

> 2026-09-08 실측(Post 721 「챗GPT 다이어트 식단」): 음식 사진 5장(`chatgpt-diet-meal-prep.webp`, `fuites-225x300.jpg`, `fresh-carrot-225x300.jpg` 등)이 전부 `UNKNOWN`으로 떨어져 수동 확인을 유발했다. 확인 결과는 **5장 모두 데코**였다 — 즉 사람이 볼 필요가 없던 건이다. ⑧단계를 넣으면 `.jpg` 2장이 자동 확정되고, 남는 `.webp` 3장만 사람이 본다.

⚠️ **⑧단계는 `.webp` 를 건드리지 않는다.** webp는 증빙일 수도 생성물일 수도 있어 판단이 갈리므로, 여전히 `UNKNOWN`으로 남겨 사람이 본다. 자동 확정 범위를 여기서 더 넓히지 않는다.
...
- `hasStockImg: true`면 **히어로 교체용 이미지 1장을 추가 생성**한다 (총 생성 = `genCount` + 1장). 이 히어로 이미지는 STEP 7-2.5에서 기존 스톡 이미지의 src/alt를 교체하는 데 사용하고, 대표이미지로도 설정한다.

🚨 **(v6.2) `genCount` 는 '본문에 새로 삽입할 장수'이고, 히어로는 그 바깥의 별도 1장이다.** 그래서 STEP 2의 공식이 **스톡을 총량에서 빼야** 최종 합계가 상한 5장에 맞는다. 舊 공식(`5 - evidence - deco`)은 스톡을 세지 않아 상한을 넘겼다.

> 2026-09-08 실측(Post 1626 「전세보증보험 집주인 거절」): `ev 2 / stk 1 / deco 0`.
> - 舊 공식 → `genCount 3` + 히어로 1 = 데코 4 + 증빙 2 = **총 6장 (상한 초과)**
> - 新 공식 → `genCount = min(3-0, 5-2-0-1) = 2` + 히어로 1 = 데코 3 + 증빙 2 = **총 5장 (정확)**
>
> 그날은 이 결함을 실행 중에 발견해 손으로 3→2로 줄여 회피했다. v6.2부터는 공식이 알아서 맞춘다.

⚠️ **자가 점검(생성 전 필수)**: `evidence + deco + stock + genCount + (hasStockImg ? 0 : 0)` 가 아니라, **최종 총량 = `evidence` + `deco` + `genCount` + `stock`** 로 센다 (스톡 자리는 히어로가 물려받으므로 개수가 유지된다). 이 값이 **5를 넘으면 생성에 들어가지 않는다** — `genCount` 를 그만큼 줄이고 STEP 8에 감축 사유를 남긴다.
...
- 따라서 `genCount = min(3 - deco, 5 - evidence - deco - stock)` 가 자동 적용된다 **(v6.2: `- stock` 추가)**. **실물 1장을 넣으면 생성이 1장 줄어든다.** 총량 상한 5장은 그대로 유지한다.
...
패널을 연 상태에서 실행한다. 버튼 텍스트가 정확히 `x2` 인 것만 고르므로 `x1 x2 x3 x4` 가 붙어 있어도 안전하다.

```javascript
// (v6.2) 설정 패널 열기 → x2 교정 → 결과 확인까지 한 호출
const bs = () => Array.from(document.querySelectorAll('button'));
const cfg = bs().find(x => /설정 트리거/.test(x.getAttribute('aria-label') || ''));
if (!/x2/.test(cfg.textContent || '')) {
  cfg.click();                                        // 패널 열기
  await new Promise(r => setTimeout(r, 1500));
  const x2 = bs().find(b => /^\s*x2\s*$/.test(b.textContent || ''));
  if (x2) { x2.click(); await new Promise(r => setTimeout(r, 1200)); }
  document.body.click();                              // 패널 닫기
  await new Promise(r => setTimeout(r, 800));
}
const now = bs().find(x => /설정 트리거/.test(x.getAttribute('aria-label') || ''));
'cfg:' + (now.textContent || '').replace(/\s+/g, ' ').slice(0, 60)
```

반환값에 `Nano Banana 2`·`crop_16_9`·`x2` 가 **모두** 있어야 통과다. `Lite` 가 섞였거나 `동영상` 이면 위 스크린샷 절차로 손수 고친다.

⛔ **(v6.2) `저장` 버튼을 찾지 않는다 — 현재 UI에는 없다.** 舊 지침의 "마지막에 저장 버튼을 누른다"는 **오류다**. 2026-09-08 실측: `x2` 클릭 즉시 칩이 `x2` 로 바뀌었고, 문서상의 `저장` 버튼은 DOM에 존재하지 않았다(`save:none`). 없는 버튼을 찾느라 시간을 쓰지 말고, **칩 텍스트로 반영을 확인**한다.
...
🆕 **(v6.2) 다만 '설정·상태 확인'이 목적이면 재촬영하지 말고 JS로 갈아탄다.** 스크린샷 타임아웃의 상당수는 **그림이 꼭 필요하지 않은 자리**에서 난다. 아래 기준으로 판단한다.

| 스크린샷의 목적 | 타임아웃 시 대응 |
|---|---|
| 설정 칩·버튼 상태·진행률 확인 | ⛔ 재촬영 금지 → **JS 반환 문자열로 대체** (5-1의 `cfg:` 확인, 5-3의 폴링) |
| 이미지 채택 판정·최종 육안 검증 | ✅ **스크린샷만 단독 재호출** (그림이 목적이므로 대체 불가) |

> 2026-09-08 실측: `x2` 클릭 직후 확인용 스크린샷이 30초 타임아웃 → 재촬영 대신 `cfg` 텍스트를 읽는 JS 한 줄로 `x2` 반영을 확인하고 **그대로 진행**했다. 회차 전체에서 스크린샷은 채택 판정 1장 + 최종 검증 1장, 총 2장만 썼다.

⛔ **재촬영은 2회까지다.** 같은 자리에서 3회 연속 타임아웃이면 렌더러가 실제로 멈춘 것이므로, Flow 탭이면 **탭을 닫고 다시 연다**(5-4의 뷰포트 고착 복구와 동일). 프리뷰 탭이면 새로고침 후 8초 대기하고 한 번만 더 시도한다.
...
- 🚨 **(v5.3) Chrome MCP의 실제 상한은 세 개다. 전부 지킨다**
  ...
  - `computer:screenshot` (CDP `Page.captureScreenshot`) — **30초**. 배치 안에서 실패하면 **스크린샷만 단독으로 한 번 더** 부른다 (2026-08-19 실측 4회 전부 재호출로 성공). **(v6.2) 단, 설정·상태 확인이 목적이면 재촬영하지 말고 JS 반환값으로 대체하고, 같은 자리에서 3회 연속 실패하면 탭을 다시 연다**
- 🚨 **(v6.2) 총량 상한 5장 계산에 `stock` 을 반드시 포함한다.** `genCount = min(3 - deco, 5 - evidence - deco - stock)`. 스톡은 히어로로 교체되면서 데코가 되므로 총량에 남는다. 舊 공식은 이를 빠뜨려 2026-09-08 Post 1626에서 **총 6장**을 만들 뻔했다. 생성 전 **`evidence + deco + stock + genCount ≤ 5`** 를 손으로 한 번 더 센다
- 🆕 **(v6.2) 분류기 ⑧단계 — WP 리사이즈본(`-225x300.jpg`)과 비-webp 사진(`.jpg`/`.png`/`.gif`)은 데코로 확정한다.** 증빙은 두 루틴 모두 `.webp` 로만 만들므로 안전하다. `.webp` 는 여전히 `UNKNOWN` 으로 남겨 사람이 본다
- 🆕 **(v6.2) Flow 설정에 `저장` 버튼은 없다.** 舊 지침의 "저장 버튼을 누른다"는 오류다. `x2` 는 5-1의 교정 JS로 클릭하고, **칩 텍스트(`Nano Banana 2 crop_16_9 x2`)로 반영을 확인**한다

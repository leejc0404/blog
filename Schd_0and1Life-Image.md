# 0and1Life 자동 이미지 삽입 태스크 (v2 — 피사체 정확성 강화)

### 목적

Notion 글 현황 테이블에서 오늘 날짜에 draft된 WordPress 글을 찾아, 이미지가 3장 미만인 글에 Gemini로 생성한 이미지 3장을 WebP 형식으로 삽입한다. 기존에 무관한 스톡(Unsplash 등) 이미지가 들어가 있으면 함께 교체한다.
...
오늘 날짜(YYYY-MM-DD 형식)와 일치하는 **"Draft일"** 행을 찾는다.
해당 행에서 **글 제목(한국어)**, **서브카테고리**, **한줄 요약**을 추출한다.

대상 글이 없으면 "오늘 날짜 draft 글 없음"을 출력하고 종료.

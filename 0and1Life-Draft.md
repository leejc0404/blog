...
> ⭐ **(v1.8 개정 2026-08-22) 주간 한도의 SSOT는 `0and1Life-Writer.md` 1-3 R1이다.** 이 표의 한도 열은 Draft 루틴이 [6o] 균형 **보고**를 할 때 쓰는 사본이며, Writer 지침이 개정되면 이 열도 같이 고친다. **WP ID·slug 매핑만 이 문서의 고유 자산**이고 한도 수치는 아니다. 두 값이 어긋나면 Writer 지침이 이긴다.

| Notion 서브카테고리 | WP ID | slug | 주간 한도 (Writer 1-3 R1 사본, 2026-08-22 기준) |
|---|---|---|---|
| 💰 직장인 재테크 | **23** | office-worker-finance | 2 |
| 💼 대기업 직장인 생활 | **29** | corporate-worker-life | 2 |
| 📐 생활 규정·계약 계산 | **30** | time-money-saving *(slug 유지)* | **2** (3→2 하향, 네이버 노출 0 실측) |
| 💍 Wedding | **27** | wedding | **2** (1→2 상향, 네이버 유입 44% 최대 클러스터) |
| 🏠 Life · My Story | **25** | daily-life | 2 |
| 🤖 AI 정보 | **26** | ai-guide | **1 (조건부 해제)** — 허용 각도·재동결 게이트는 Writer 1-3 |
...
```
R1. 7일 창 동일 카테고리 상한: 재테크 2 · 대기업 2 · 규정계산 2 · 웨딩 2 · Life 2 · AI 1
    (v1.8 2026-08-22 재조정 — SSOT는 Writer 1-3 R1. 규정계산 3→2 · 웨딩 1→2 · AI 0→1)
R6. 네이버 수요 하한 — 7일 창에 `N-통과`가 2건 미만이거나 `N-구글전용`이 4건 이상이면
    보고에 `⚠️ 네이버 수요 하한 미달` 을 명시한다. (배정은 Writer 루틴의 책임 — Draft는 보고만)

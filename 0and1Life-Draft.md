...
### 개정 이력
- **v1.9 (2026-09-07)** — **균형 규칙 사본 폐지.** 7-1 표의 `주간 한도` 열과 7-3의 R1~R6 사본을 **삭제하고 `0and1Life-Writer.md` 1-3 R1~R6 직접 조회로 전환**했다. 근거: 2026-09-07 Draft 회차가 `📐 규정계산 3건 — 한도 2 초과`를 보고했는데, 사본에 Writer 1-3 R1의 **「Head Type=시즌 글은 R1 카운트에서 제외」 예외가 누락**돼 있어 오탐 가능성이 확인됐다(사본 생성 이후 Writer에 추가된 조항이 따라오지 않음). 같은 이유로 사본에 없던 **R4·R5·7-4 F등급 가드**도 스케줄 프롬프트 폴백에서 소실되고 있었다. **WP ID·slug 매핑은 이 문서의 고유 자산이므로 그대로 유지**한다. 조항 번호(7-1·7-3·7-4)는 보존했다.
- **v1.8 (2026-08-22)** — **Phase 7 카테고리 한도 재조정 + SSOT 명시.** 2026-08-22 네이버 서치어드바이저 실측(클릭 8회 -91.6% · 노출 5.6천 -73.7% · 노출 상위 5문서 중 4개가 6/20~7/4 작성분)에 근거해 📐 규정·계산 3→2, 💍 웨딩 1→2, 🤖 AI **0(동결)→1(조건부 해제)** 로 조정. 7-1 한도 열이 **Writer 지침 1-3 R1의 사본**임을 명시하고 상충 시 Writer가 이긴다는 우선순위를 못 박았다(7-1 표와 7-3 R1이 규정계산 2 vs 3으로 서로 어긋나 있던 내부 모순도 함께 해소). 7-3에 **R6 네이버 수요 하한** 보고 항목 신설.
- **v1.7 (2026-08-17)** — Phase 5-4 관문 ①에 **「⛔ 본문 삽입 금지 자료」(SEO 리서치 산출물)** 조항 신설. KoreaPlug-Draft v10.5와 동일 기준.
...
### 5-3-C. `keywordDensity` 하한 미달 — Draft 루틴은 조치하지 않는다 (자동 신설 2026-09-08, 루틴 작성)

> **신설 사유**: 2026-09-08 회차(#104 `jeonse-guarantee-landlord-rejection`)에서 Rank Math 실패 항목으로 `keywordDensity`가 떴는데, 5-3-B 판정표와 스케줄 프롬프트 [6j] 어디에도 이 항목의 행이 없었다. Phase 5-0-A 3단 검증(`?cb=` 재조회 → Claude in Chrome `fetch` 재조회, 둘 다 v1.9 최신본 확인)을 모두 밟은 뒤 **결번으로 확정**했고, 5-0-B에 따라 루틴이 직접 작성한다.

| 실패 항목 | 판정 | 조치 |
|---|---|---|
| `keywordDensity` (하한 미달) | ⛔ **소관 밖 — Draft 루틴 조치 불가** | 보정하지 않고 로그에 실측 밀도를 기록하고 넘어간다 |

**판정 근거**

1. **유일한 해소 수단이 이미 금지돼 있다.** 밀도 하한을 올리는 방법은 본문에 Focus Keyword를 더 넣는 것뿐인데, 스케줄 프롬프트 [6j]는 `⛔ 절대 금지: … 키워드 반복 삽입` 과 `키워드 기계적 반복 삽입 금지` 를 명시한다. 금지된 조치만 남은 항목은 조치 가능 항목이 아니다.
2. **자매 지침서에도 하한 조치 규정이 없다.** `KoreaPlug-Draft.md` 오류표는 `Keyword 밀도 경고 | 2.2% 초과 | 본문에서 Focus Keyword 2~3회 삭제` 로 **상한 초과만** 다룬다. 하한 미달은 양쪽 지침서 모두 미규정 상태였다.
3. **하한 미달은 작성 단계의 문제다.** `0and1Life-Writer.md` 2-5가 Focus Keyword 밀도 **1.0~1.5%** 를 작성 기준으로 두고 있다. 이 기준에 못 미친 원고가 넘어온 것이므로 책임 소재는 Writer이고, Draft 루틴은 **보고만** 한다(Phase 7 [6o] 카테고리 균형과 같은 취급).

**실측 (2026-09-08 #104)**: Rank Math 패널 `키워드 밀도가 0.37로 낮으며 포커스 키워드와 조합이 6번 나타납니다` · 루틴 자체 계산 0.39%(본문 1,777어절 중 Focus Keyword 7회). Writer 2-5 하한 1.0%의 절반 이하.

**보고 형식**: STEP 7 완료 알림에 `⚠️ keywordDensity {n}% — Writer 2-5 하한(1.0%) 미달, 작성 단계 소관` 을 한 줄 덧붙인다. 조용히 넘어가지 않는다.

⚠️ **상한 초과(2.2% 초과)는 이 조항의 대상이 아니다.** 그쪽은 기존 오류표대로 본문에서 Focus Keyword를 2~3회 삭제해 조치한다 — 삭제는 금지 대상이 아니기 때문이다.
...
> ⛔ **(v1.7 신설 2026-08-17) 본문 삽입 금지 자료 — SEO 리서치 산출물**
...
>
> **판정 질문 한 줄: "독자가 이 캡처를 보고 행동을 바꾸는가?"** 아니오면 넣지 않는다. 잘 찍은 자동완성 캡처도 반려 대상이다.
> **대신 넣는 것**: 정부·공공기관 조회 결과, 요금·운영시간 안내, 공식 지원 목록, 법령 조문, 신청 서식·예약 화면.
> 배경: 자매 블로그 KoreaPlug 2026-08-16 회차에서 자동완성 캡처를 관문 ① 근거로 두 글에 넣었다가 사용자 검토에서 반려됐다(KoreaPlug-Draft v10.5와 동일 조항). 이 블로그 지침에는 해당 인정 문구가 원래 없었으나, 같은 실수를 막기 위해 금지를 명시한다.
...
> ⭐ **(v1.9 개정 2026-09-07) 이 표에는 주간 한도를 두지 않는다.** v1.8까지 있던 `주간 한도` 열은 `0and1Life-Writer.md` 1-3 R1의 사본이었는데, Writer에 뒤늦게 추가된 예외 조항(시즌 제외 등)이 따라오지 않아 오탐을 만들었다. **한도·균형 판정은 7-3의 지시대로 Writer 1-3 R1~R6을 직접 조회해서 한다.**
> **이 표의 고유 자산은 `Notion 서브카테고리 → WP ID → slug` 매핑뿐이다.** 이것만 이 문서가 소유한다.

| Notion 서브카테고리 | WP ID | slug |
|---|---|---|
| 💰 직장인 재테크 | **23** | office-worker-finance |
| 💼 대기업 직장인 생활 | **29** | corporate-worker-life |
| 📐 생활 규정·계약 계산 | **30** | time-money-saving *(slug 유지 — 이름과 불일치하지만 변경 금지)* |
| 💍 Wedding | **27** | wedding |
| 🏠 Life · My Story | **25** | daily-life |
| 🤖 AI 정보 | **26** | ai-guide *(허용 각도·재동결 게이트는 Writer 1-3)* |
| 0. 테크와 효율 | 18 | tech-efficiency *(사용 중단 — 레거시, 신규 배정 금지)* |
...
### 7-3. 편중 방지 규칙 — Writer 직접 조회 (v1.9 전환)

⛔ **여기에 R1~R6을 옮겨 적지 않는다.** v1.8까지 두었던 사본은 삭제했다.

**판정 절차 (Draft 루틴 [6o]가 수행)**

1. `https://raw.githubusercontent.com/leejc0404/blog/main/0and1Life-Writer.md?cb={epoch_ms}` 를 조회한다. (`?cb=` 필수 — 5-0-A와 같은 캐시 사고를 막는다)
2. `**카테고리 균형 규칙 (R1~R6)**` 이후 코드블록 **전문**을 읽는다.
3. **상한 숫자만 떼어 읽지 않는다.** R1에 딸린 예외 조항을 반드시 함께 적용한다:
   - `Head Type=시즌` 글은 R1 카운트에서 **제외**한다 (이슈·제도는 제외 대상 아님)
   - 하루 N건 모드(DAILY_TARGET≥2)면 상한을 N배로 읽는다 (AI는 배수 제외)
   - R2-D: 같은 날 두 글은 카테고리가 달라야 한다
   > 이 세 줄은 **판정 항목이 아니라 "무엇을 놓치기 쉬운지"에 대한 안내**다. 실제 값과 조항 전문은 항상 Writer에서 읽는다.
4. 판정 결과를 STEP 7에 **보고만** 한다. **이 루틴은 카테고리를 바꾸지 않는다** — 배정은 Writer 루틴의 책임이다. 조용히 넘어가지 말 것.

**조회 실패 시 (폴백)**

- 저장소 루트 1회 → Claude in Chrome 1회 재시도
- 그래도 실패하면 **낡은 사본으로 판정하지 말고 판정을 생략**하고, STEP 7에 `⚠️ 카테고리 균형 판정 불가 — Writer 지침 접근 실패` 를 명시한다.
- ⛔ 기억에 남은 수치나 다른 문서의 사본으로 대신 판정하지 않는다. **불완전한 판정이 판정 없음보다 나쁘다** — 2026-09-07 오탐이 그 사례다.

> ⚠️ **7-4 발행량 가드(F등급)는 이 문서의 고유 조항이 아니다.** Writer 1-3 '발행량 가드'와 같은 기준이며, 수치가 어긋나면 Writer가 이긴다.

> 📌 **아래 두 문단은 규칙이 아니라 2026-08-08 시점의 기록이다.** 안에 적힌 한도 수치(예: '한도 3')는 이후 v1.8에서 바뀌었으므로 **판정에 쓰지 않는다.** 현재 값은 항상 Writer 1-3 R1에서 읽는다.

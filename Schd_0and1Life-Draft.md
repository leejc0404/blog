...
⚠️ **조회 URL에는 반드시 `?cb={epoch_ms}` 를 붙인다.** 2026-08-16 KoreaPlug 실행에서 캐시된 구버전이 반환돼 실재하는 관문 조항을 "없다"고 오판하고 허위 상충을 보고한 사고가 근거다.
fetch 실패면 이 프롬프트의 규칙만으로 진행하고 오류 로그에 "GitHub 지침 접근 실패 — 폴백 사용"이라고 기록한다.

⚠️ [조항 자기보완 규약 — 2026-08-16 신설, 지침서 Phase 5-0]

> ⚠️ **이 파일은 사본입니다.** 실제로 실행되는 원본은 Cowork 스케줄 태스크 `0and1life-wordpress-draft` 의 프롬프트이며, 이 파일을 고쳐도 실행에는 반영되지 않습니다. 지침을 바꾸려면 `mcp__scheduled-tasks__update_scheduled_task` 로 프롬프트를 갱신하고 그 내용을 이 파일에 동기화하세요.
> **최종 동기화: 2026-08-08 (v2 — 저장순서 규약·카테고리 매핑 정정)** · 스케줄: 매일 06:14 KST (`10 6 * * *`)
...
⚠️ [무인 실행 원칙 — 최우선]
이 루틴은 사람이 없는 새벽에 실행된다. **사용자 승인이 필요한 도구 호출을 불필요하게 하지 않는다.** 2026-08-06 실행이 STEP [3-0]의 불필요한 `request_cowork_directory` 승인 창에서 13시간 정지한 사고가 근거다. 승인이 필요한 호출은 "그것 없이는 진행이 불가능함을 먼저 실측으로 확인한 뒤"에만 한다.

⚠️ [최우선 규약 — 2026-08-08 신설] 저장 순서와 자동저장 사고
**에디터 탭이 열려 있는 동안에는 REST로 본문을 쓰지 않는다.**
근거(2026-08-08 실측): draft는 Gutenberg 자동저장이 리비전이 아니라 **원본 글에 직접 기록**된다. 에디터 탭을 열어둔 채 REST로 본문을 덮어쓰면 에디터 메모리의 구버전이 REST 결과를 되덮어쓴다. 실제 피해: `<p>` 56개 → 2개, 18,353자 → 17,984자.
...
STEP 0에서 매번 `web_fetch`로 실시간으로 불러와 세션 내 재사용한다. 캐시·고정하지 않는다. fetch 실패면 이 프롬프트의 규칙만으로 진행하고 오류 로그에 "GitHub 지침 접근 실패 — 폴백 사용"이라고 기록한다.

⚠️ 이 프롬프트가 실행되는 유일한 원본이다. `C:\Users\win\.claude\blog\Schd_0and1Life-Draft.md` 는 사본일 뿐이다.

STEP 0 — 지침서 읽기 (최우선, 세션당 1회)
fetch → https://raw.githubusercontent.com/leejc0404/blog/main/0and1Life-Draft.md
목적: ① Phase 5-1 저장 순서 규약 ② Phase 5-3-B 조치 가능 여부 판정표 ③ Phase 7 카테고리 배정 ④ Phase 5-4 ⛔ 발행 차단 관문.
...
[3-4] 서브카테고리 → WP Category ID (⭐ 2026-08-08 REST 실측 정정)
⚠️ 舊 매핑표는 실제와 어긋나 있었다. AI 계열을 18번으로 배정하도록 돼 있었으나 실제 AI 글 15건은 전부 26번(ai-guide)에 있고 18번(tech-efficiency)에는 4건뿐이다.

💰 직장인 재테크 / 재태크 → 23 (office-worker-finance)
💼 대기업 직장인 생활 → 29 (corporate-worker-life)
📐 생활 규정·계약 계산 (舊 ⏱️ 시간·돈 절약 시스템) → 30 (time-money-saving)
💍 Wedding → 27 (wedding)
...
⚠️ **답변 텍스트는 첫 문장만 넣는다** (2026-08-08 규정). 전문을 넣으면 200어절을 넘고 Rank Math가 본문 문단으로 계산해 잡음이 된다. 스키마 유효성에는 영향 없다.
Q/A가 1쌍 이하면 null.
...
캡처 대상 선정 (3회 검증):
- ✅ 법령(法令) 조문은 캡처 가능 — `https://www.law.go.kr/법령/{법률명}/제{n}조` 는 `lawService` iframe에 HTML 인라인 렌더링된다. 8/4 근로기준법 시행령 제6조 · 8/6 방문판매법 제32조 · 8/8 민법 제623조 3회 검증. **1순위 경로.**
...
- ⚠️ 표준 우회로 (4회 검증): 대상 블록을 복제해 `position:fixed;inset:0;background:#fff` 오버레이로 뷰포트를 채운 뒤 전체 스크린샷.
...
화자 프로필(2026-08-05 사용자 확정): 미혼 · 2026-11-01 결혼 예정 · 예식장 계약 완료 · 스드메 상담 3곳 · **상견례 미완료** · **신혼집 입주 전** · **자녀 없음(육아휴직 경험 없음)** · 직장 10년차 이상 · AI는 Claude(Max)·Gemini(Pro)·ChatGPT·퍼플렉시티 실사용.
...
⚠️ TOC 강제 인식을 **이 단계에서 미리** 넣는다. 이것만으로 TOC 항목은 통과한다 (8/6·8/8 검증).
...
[6h] 저장 (⭐ 2026-08-08 개정 — 이 STEP의 핵심)
...
[6j] SEO 점수 확인 및 보정 (⭐ 2026-08-08 전면 개정)

에디터 새로고침 → wait 3초 → `wp.data.select('rank-math').getAnalysisScore()`. 목표 81점 이상이면 [6k]로.

**81점 미달 시 — 보정 전에 조치 가능 여부부터 판정한다.**

⛔ **재분석 트리거를 반사적으로 돌리지 말 것.** 2026-08-08 실측: 서버 저장 점수가 79점이었는데 재분석 트리거를 돌리자 클라이언트 점수 76점이 메타를 덮어썼다. 클라이언트 분석은 `getEditedPostContent()` 를 쓰는데 이 값은 freeform 직렬화에서 `<p>`가 소실된 상태(59개 → 2개)라 구조적으로 불리하다.
...
⛔ **`contentHasShortParagraphs` 는 문단을 쪼개도 해소되지 않는다 (2026-08-08 규명).**
저장본 raw가 `<p>` 56개·최장 문단 40어절로 정상인데도 실패한다. Rank Math가 보는 `getEditedPostContent()` 에서 `<p>`가 소실돼 본문을 통짜 덩어리로 계산하기 때문이다. 8/7의 `<p>` 복원, 8/8의 FAQ ld+json 축약(223→47어절) 모두 해소하지 못했다.
...
[6m-0] ⭐ 탭 정리 절차 (REST 본문 수정 전 필수 — 2026-08-08 신설)
...
[6o] 카테고리 균형 점검 (2026-08-08 신설 — 보고 전용)
현황표에서 **최근 7일 발행분의 카테고리 분포**를 센다. 아래에 해당하면 STEP 7에 명시한다:
- 7일 창 상한 초과? (재테크 2 · 대기업 2 · 규정계산 3 · 웨딩 1 · AI 0)
- 직전 3건이 같은 카테고리? (3연속 금지 위반)
- 7일 창에 서로 다른 카테고리가 3개 미만?
⚠️ 이 루틴은 카테고리를 바꾸지 않는다 — **초과 사실을 보고만 한다.** 배정은 Writer 루틴의 책임이다. 조용히 넘어가지 말 것 (2026-08-03~08 절약 5연속이 무보고로 진행됐다).
...
GitHub 지침 fetch 실패 | 이 프롬프트의 [4f]·[6j] 규칙만으로 진행 + 오류 로그 기록

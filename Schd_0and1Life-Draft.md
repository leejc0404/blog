...
[0-6] Writer 지침 조회 — 카테고리 균형 규칙 SSOT (v2 신설 2026-09-07)
fetch → https://raw.githubusercontent.com/leejc0404/blog/main/0and1Life-Writer.md?cb={epoch_ms}
목적: **[6o] 균형 판정에 쓸 R1~R6 전문**. `**카테고리 균형 규칙 (R1~R6)**` 이후 코드블록을 통째로 읽어 세션 내 보관한다.
⛔ **이 프롬프트에는 한도 수치를 적어두지 않는다.** 사본은 Writer 개정을 따라오지 못해 오탐을 만든다 — 2026-09-07 회차에서 R1의 「시즌 글 제외」 예외가 빠진 사본으로 `📐 3건 초과`를 오보고한 사례가 근거다.
⚠️ **상한 숫자만 떼어 읽지 않는다.** R1에 딸린 예외(시즌 제외 · 하루 N건 배수 · R2-D)와 R2~R6, 그리고 '발행량 가드(F등급)' 문단까지 함께 읽는다.
조회 실패 시: 저장소 루트 1회 → Chrome 1회 재시도 → 그래도 실패면 **[6o] 판정을 생략**하고 STEP 7에 `⚠️ 카테고리 균형 판정 불가 — Writer 지침 접근 실패` 를 기재한다. 기억이나 다른 문서 사본으로 대신 판정하지 않는다.
※ [0-1] Draft 지침서 조회와 별개다. 둘 중 하나만 실패해도 나머지는 정상 진행한다.
...
[6o] 카테고리 균형 점검 (보고 전용 — v2 개정 2026-09-07)
판정 기준은 **[0-6]에서 읽어둔 Writer 1-3 R1~R6 전문**이다. 이 프롬프트에는 수치를 두지 않는다.

1) 현황표에서 최근 7일(오늘 포함) 글의 **카테고리 · Head Type · 네이버 검증 등급(N-통과/N-구글전용) · 형식**을 뽑는다.
   ⚠️ Head Type을 같이 뽑아야 한다 — R1의 시즌 예외를 적용하려면 필요하다. 현황표에 없으면 서브페이지 기본 정보 표에서 읽는다.
2) [0-6]의 R1~R6을 **예외 조항까지 포함해** 그대로 적용해 위반 항목을 가린다.
3) 위반이 있으면 STEP 7에 `⚠️ 카테고리 균형 — {규칙번호}: {내용}` 형식으로 명시한다. 어떤 규칙(R1~R6) 위반인지 번호를 반드시 적는다.
4) R1 초과를 보고할 때는 **시즌 예외를 적용한 뒤의 카운트인지 함께 밝힌다.** 예: `R1: 📐 3건(시즌 제외 후 3건) — 한도 2 초과`

⚠️ 이 루틴은 카테고리를 바꾸지 않는다 — **위반 사실을 보고만 한다.** 배정은 Writer 루틴의 책임이다. 조용히 넘어가지 말 것.
⚠️ [0-6] 조회에 실패했으면 판정을 생략하고 `⚠️ 카테고리 균형 판정 불가 — Writer 지침 접근 실패` 만 기재한다. **낡은 사본·기억으로 대신 판정하지 않는다.**
⚠️ 발행량 가드(F등급)도 Writer 소관이다. [0-6]에서 읽은 '발행량 가드' 문단 기준으로 해당되면 STEP 7에 `⚠️ F등급 {n}% — 정리 우선 구간` 을 덧붙인다.
...
⚠️ STEP 0 결과를 한 줄로 함께 기록한다 — 조회 실패([0-5]) / 캐시 사고([0-3]) / 조항 자동 신설([0-4], 조항번호 + 전문) / Writer 조회 실패([0-6]) / 진짜 상충. 캐시 사고를 상충으로 쓰지 않는다.
신규 없음: "0and1Life 자동 체크 완료 — 신규 글 없음 ({TODAY_KST})"

아래 해당 시 각각 한 줄 덧붙인다:
- [6o] 균형 위반 → "⚠️ 카테고리 균형 — {규칙번호}: {내용}" (R1 초과는 시즌 예외 적용 여부 병기)
- [6o] 판정 불가 → "⚠️ 카테고리 균형 판정 불가 — Writer 지침 접근 실패"
- [6o] F등급 가드 해당 → "⚠️ F등급 {n}% — 정리 우선 구간"
...
Writer 지침([0-6]) fetch 실패 | 저장소 루트 1회 → Chrome 1회 → 실패 시 **[6o] 판정만 생략**하고 draft 작업은 정상 진행. STEP 7에 "카테고리 균형 판정 불가" 기재. 사본·기억으로 대신 판정 금지
균형 규칙 수치가 필요한데 Writer를 못 읽음 | 판정하지 않는다. 불완전한 판정이 판정 없음보다 나쁘다 (2026-09-07 시즌 예외 누락 오탐 사례)

...
**v10.25(2026-09-04) 정비 — 구조 무결성 개정 (이번 개정의 핵심)**: 발행 142편 전수검사에서 **이 루틴이 만들어 온 구조 결함 3종**이 확인됐다. 원인은 전부 `[6m]` 내부 링크 삽입 로직이다.
- 🔴 **Related reading 칼럼 이탈 32편** — 舊 `[6m]`의 "마지막 `</ul>` 직전에 삽입 / 섹션이 없으면 신설"이 섹션을 **본문 래퍼(`max-width` + `margin:auto`) 밖**에 만들었다. 래퍼 밖은 전체 폭 기준이라 이 블록만 왼쪽 끝에 붙어 렌더링된다. 라이브 실측: `tipping-in-korea` 삽입 지점 div 깊이 **−6**, `korea-internet-setup` **0**(0 이하 = 래퍼 밖). → `[6m]`을 **래퍼 내부 판정 필수**로 전면 재작성하고 `[6m-R]` 복구 절차를 신설했다.
- 🟡 **미발행 글 선링크 15종** — 舊 `[6m]`은 삽입 대상(기존 발행 글)만 검사하고 **링크 목적지(신규 글)의 발행 상태를 보지 않았다.** 신규 글이 `future`인 동안 그 링크는 전부 404이며, 404가 사이트 전체 크롤에 누적된다. → **목적지 `publish` 확인 게이트**와 `[6m-Q]` 예약 큐를 신설했다.
- 🔴 **본문 전체 중복 렌더 1편**(`zero-waste-cafes-seoul`) — 본문 안에 `wp:post-content` 블록이 중첩돼 글이 두 번 출력된다(라이브에서 본문 문장 2회 확인). → `[5-0]` 업로드 전 정규화에 **금지 블록 검사**를 넣었다.

부수 개정: `[5-0]` 신설(업로드 전 HTML 정규화·검증), `[6n]` 신설(배포 후 구조 검증 9종), 오류 처리 표의 **reauth 자기모순 제거**(STEP 3 ③은 "로그인 버튼 클릭 금지"인데 오류표는 "클릭"으로 남아 있었다), 부록 B 신설(판정식 원문).

**v10.24(2026-08-16) 정비**: STEP 0을 **조회 + 무결성 검증** 2단으로 재작성했다. 2026-08-16 실행에서 raw 조회가 **캐시된 v10.2 사본**을 반환해, 지침서에 실재하는 5-4·5-4B를 "없다"고 오판하고 [부록 A] 폴백으로 관문을 판정한 뒤 허위 상충을 보고한 사고가 근거다(브라우저 캐시버스터 재조회로 조항 정상 확인). 앞으로 ①모든 조회에 `?cb=` 필수 ②조항이 안 보이면 3단 검증 ③결번 확정 시 **조항을 직접 작성해 로컬 지침서 md에 써 넣고 배포 계속**(지침서 5-0-B). 커밋·푸시는 사용자가 직접 한다.
...
⚠️ **`?cb=` 없이 조회하지 않는다.** 2026-08-16 실행에서 캐시된 v10.2 사본이 반환돼, 실재하는 5-4·5-4B를 "없다"고 오판하고 허위 상충을 보고했다.
...
2. **반영** — `C:\Users\win\Documents\Claude\blog\KoreaPlug-Draft.md` 에 **직접 써 넣는다.** 조항 제목에 `(자동 신설 {TODAY}, 루틴 작성)` 표기.
   - 폴더가 연결돼 있지 않으면 `mcp__cowork__request_cowork_directory { path: "C:\Users\win\Documents\Claude" }` 로 먼저 연결한다 (STEP 3 ⓪과 동일 단계).
...
> ⚠️ v10.25 변경: 후보가 없는 날에도 `[6m-Q]` 인바운드 링크 예약 큐는 비워야 한다. 신규 글이 발행(`future` → `publish`)되는 날은 대개 신규 후보가 없는 날이기 때문이다.
...
> ⭐ **v10.25 추가 — Basic Auth가 살아 있으면 STEP 5·6m·6n은 curl로 처리한다.** 2026-09-03 실측에서 앱 비밀번호 Basic Auth로 `GET/POST /wp-json/wp/v2/posts` 가 정상 동작했다(142편 조회·본문 수정 성공). **Rank Math 설정(`[6c]`~`[6g]`)만 브라우저 UI가 필요**하고, 나머지 REST 작업은 브라우저 없이 끝난다. 브라우저 세션이 죽어도 구조 검증(`[6n]`)과 큐 처리(`[6m-Q]`)는 계속 수행할 것.
...
- ⛔ **루틴은 로그인 폼을 대신 제출하지 않는다.** 자격증명 입력과 인증 폼 제출은 AI 금지 동작이며, **아이디·비밀번호가 자동완성돼 있어도 '로그인' 버튼을 클릭하지 않는다.** (舊 조항은 자동완성 시 클릭을 허용했으나, 2026-08-03 실행에서 이 동작이 거부되어 조항을 정정했다.)
...
- **응답에 데이터가 있으면**(기존 WP_POST_ID 확보) 곧바로 건너뛰지 말고 아래를 확인한다:
  ...
  3. **⭐ v10.25 신설 — 구조 결함이 남아 있는가** → `[6n]` 검사 9종을 그대로 1회 돌린다. 하나라도 걸리면 미완성으로 본다.
...
> ⭐ **v10.25 기록 항목 추가**: 본문에 `images.unsplash.com` 직링크가 남은 채로 배포되면 **WP 미디어에 사본이 없는 상태**다(2026-09-03 전수검사: 32편). 현재 전부 200이지만 외부 CDN에 의존하는 자산이므로, 배포 후 오류 로그에 `Unsplash 핫링크 {n}건 — 이미지 루틴 업로드 대상`으로 남긴다. 교체는 이미지 루틴 소관이며 이 루틴은 **기록만** 한다.
...
### [5-0] ⭐ 업로드 전 HTML 정규화·검증 (v10.25 신설 — 업로드 POST 직전 필수)
...
> 근거: 2026-09-03 전수검사 — `zero-waste-cafes-seoul`(ID 1101)이 `wp:post-content` 중첩으로 **본문 전체를 두 번 출력**하고 있었다. H1 2개·래퍼 id `k-zero-report` 2개. 라이브에서 본문 문장 `"quietly reshaping the city"` 2회 확인.
...
`href="https://koreaplug.com/{slug}/"` → `href="/{slug}/"` 로 치환한다. 지침서 `2-1`이 상대경로를 규정하고 있고, 절대경로는 외부 링크로 인식돼 SEO 감점이다.
> 근거: 2026-09-03 전수검사 — 발행 글 17편에 절대경로 내부링크 잔존.
...
```
> 근거: 2026-09-03 전수검사 — 4열 이상 표를 가진 29편에 래퍼가 없어 모바일에서 표가 화면 밖으로 넘친다(`chuseok-bank-holiday` 6열, `chuseok-palace-hours`·`seoul-during-chuseok`·`boknal-meaning` 5열).
...
⚠️ 보정으로 HTML을 수정했으면 **[5-0] 1)~6) 검사를 다시 1회 돌린다.** 보정이 구조를 깨뜨리는 사고를 막는다.
...
- **⭐ [6n] 구조 검증에서 🔴 항목이 남아 있음 (v10.25 추가)**

성공했다면 메인 테이블 "draft 일자" 셀 → TODAY. ←인 컬럼도 [6m] 결과로 갱신.

---

### [6m] 들어오는 내부 링크 — ⭐ v10.25 전면 재작성 (지침서 6-3)

> ⛔ **舊 조항의 결함**: "마지막 `</ul>` 직전에 삽입 / Related reading 섹션이 없으면 신설"이라고만 적혀 있었다. `content.raw` 의 마지막 `</ul>` 은 **본문 래퍼가 닫힌 뒤**에 있는 경우가 많고, 섹션을 신설하면 문서 맨 끝(= `<!-- /wp:html -->` 뒤)에 붙는다. 래퍼 밖은 `max-width`·`margin:auto` 가 적용되지 않아 그 블록만 전체 폭 왼쪽 끝에 렌더링된다. **이 한 줄이 32편을 깨뜨렸다.**
...
> 근거: 2026-09-03 전수검사 — 미발행 글로 향하는 선링크 **15종**이 살아 있었다(`carry-passport-in-korea`·`korea-arrival-card-transit`·`yeot-korean-title-meaning` 등, 각 2편에서 참조). 예약 발행일까지 전부 404이고 그 404가 사이트 전체 크롤에 누적된다. 舊 `[6m]`은 삽입 **대상** 글만 검사하고 링크 **목적지**의 상태를 보지 않았다.
...
- 신규 후보가 0건인 날에도 이 큐는 처리한다(STEP 2 말미 참조).
...
- ⛔ **`lastIndexOf('</ul>')` 를 삽입 기준으로 쓰지 않는다.** 이것이 舊 조항의 결함이다.
...
- 한 회차에 복구하는 글 수는 **최대 5편**으로 제한한다(일괄 사고 방지). 나머지는 다음 회차로 이월하고 STEP 7에 잔여 건수를 적는다.

---

### [6n] ⭐ 배포 후 구조 검증 (v10.25 신설 — STEP 6 마지막, 필수)
...
> 근거: 2026-09-03 전수검사(발행 142편) — 🔴 칼럼 이탈 32편 · 금지 블록 1편. 🟡 선링크 15종 · 절대경로 17편 · 표 래퍼 없음 29편 · Unsplash 핫링크 32편. 반면 태그 개폐 균형·플레이스홀더·모지바케·죽은 슬러그·이미지 404 는 **전부 0건**이었다 — 즉 이 루틴이 못 잡던 것은 정확히 위 표의 항목들이다.

---

## STEP 7 — 완료 알림 출력 (200자 이내, 이전 단계 성공 여부와 무관하게 반드시 실행)

- 신규 포스트 있음: `KoreaPlug 발행완료 ✅ draft {N}개 | 관문반려 {R}건(로그 기록) | 인바운드링크 {L}건 | 구조검증 {P}/{T} | SEO평균 {점수}점 | 오류 {E}개 ({TODAY_KST})`
...
⚠️ **v10.25 추가 보고 항목**: `[6m-Q]` 인바운드 대기 잔여 건수 / `[6m-R]` 구조 복구 편수와 이월 잔여 / `[6n]` 🔴 미해소 슬러그 목록.
...
## [부록 B] ⭐ 구조 판정식 (v10.25 신설 — [5-0]·[6m-2]·[6n] 공통)
...
삽입 지점 판정용:
  ...
  depth >= 1 이어야 래퍼 안이다
```
> 실측 대조값(2026-09-03): `tipping-in-korea` depth **−6**, `korea-internet-setup` depth **0** — 둘 다 래퍼 밖.
...
> ⚠️ 판정식은 **원본 문자열 기준**이다. 렌더된 페이지로 판정하면 플러그인이 주입한 마크업(EZ TOC·저자 박스 등)이 섞여 오탐이 난다. 실제로 `<!-- TABLE OF CONTENTS -->` 주석 33편은 EZ TOC가 목차를 따로 생성하므로 **결함이 아니고**, CSS 배경 히어로를 쓰는 3편은 `<img>` 가 0개인 것이 정상이다. 이 둘을 결함으로 잡지 않는다.

[루틴 : KoreaPlug-Writer — v10.0 · 2026-09-21]
...
**판정 기준·수치·품질 기준의 단일 기준(SSOT)은 전적으로 GitHub 지침서**(`KoreaPlug-Writer.md`, **v13.0 이상**)다.
...
- **지침서**: `https://raw.githubusercontent.com/leejc0404/blog/main/KoreaPlug-Writer.md?cb={epoch_ms}` — WebFetch가 인용을 거부하면 `curl`로 원문. 404/403이면 저장소 루트 1회 재확인 후 실패 시 **STOP** (추측으로 지침 생성 금지)
  → **버전 확인**: 상단이 **`v13.0` 이상**이고 `0-0 두 방향·두 금지` · `0-1 엔진 G·D·C` · `0-2 관문 3개` · `0-3 채점 3축` · `0-5 업그레이드 회차` · `0-9 GSC·GA4 규격` · `1-1 승산 판정 (가)~(라)` · `1-2 경쟁글 H2 대조` 가 있는가? 없으면 구버전 — STEP 8에 SSOT 이슈로 기록하고 사용자에게 알린다
...
3. 남은 쿼리마다 걸린 페이지의 **라이브 H2**를 읽고 판정:
   - 본문이 이미 답을 갖고 있다 → **(a) 업그레이드 후보** (`UPGRADE_LEFT > 0`일 때만; 0이면 백로그 '업그레이드 대기'로 적재)
...
1. 지침서 시드 목록에서 **최소 8개**를 골라(직전 3회차와 겹치지 않게 순환) 자동완성 조회
2. 변형 8개 이상 + `reddit`/`for tourists`/`for foreigners` 변형 1개 이상인 시드만 후보화. 변형 문구 그대로 기록
...
1. STEP 0 ②에서 정지되지 않고 7일 간격을 통과한 이벤트만
...
- (다) 0건 또는 (라) AIO 완결 → 탈락 → 2순위로 1회 더. 둘 다 탈락이면 예비 리스트 → 그래도 없으면 STOP
...
| ⚙️ 엔진 | G {후보 n · (a) n · (b) n} / D {시드 n · 후보 n} / C {후보 n} — 생략 엔진과 사유 |
...
| 지침서 버전 | **v13.0 이상** (`0-0`~`0-9` · `1-0`~`1-6` 신설 구조). 미충족 시 STEP 8에 SSOT 이슈 + 사용자 알림 |

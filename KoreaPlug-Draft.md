...
- **포함**: 5-2(TOC 강제 스크립트) → 5-3(블록 구조·820px·키워드 입력 체크) → **5-4(⛔ 발행 차단 관문)** → **5-4B(캡처 품질·마크업)** → 5-4C(피드백) → 5-5(Schema) → Phase 6(완료 처리) → 오류표 → Phase 7(카테고리 매핑)
...
- 직접 센 수치 (구글 자동완성 제안 개수 실측, 특정 조건의 항목 수 카운트 등)
...
- ✅ "On 8 August 2026 I queried Google's autocomplete endpoint…" (**실제로 수행한 작업**이므로 허용)

...
- `snapshots/{날짜}.json` — 날짜별 TOP30 · `demand/` — 날짜별 수요 · `backups/` — 수정 전 원본 · `log.md` — 사람이 읽는 기록 · `inbox/` — 수동으로 넣은 CSV · `tests/` — 엔진 테스트
...
⚠️ 변경 금지 항목: 글의 status(publish/draft/private)·**슬러그(URL)**·카테고리·발행일·**기존 H2 문구와 순서**·첫 문단과 답 블록(STEP 7-5) 외 본문.
- Focus Keyword는 **1순위만** 수요 1위 키워드(`focus_kw`)로 바꿀 수 있다. 기존 키워드는 지우지 않고 뒤로 보낸다.
...
| C 순위·노출 영역 문제 | `share` < 15% (검색은 많은데 우리가 거의 안 보임) 또는 상위10을 기관·언론이 채움 | B 처방 + **본문 답 블록(7-5)** + Focus Keyword 1순위를 `focus_kw` 로 |
...
2) python으로 `content.raw` 의 **style 없는 첫 `<p>…</p>`** 하나만 새 문단으로 바꾸고, 답 블록이 있으면 7-5 위치에 넣어 `POST /wp-json/wp/v2/posts/{id}` `{"content": ...}`. 원래 문단에 기대한 문구가 있는지, 삽입 기준 표시가 정확히 1개인지 assert 후 교체한다.
...
```
```javascript
// ② 반영 — savePost 는 한 번만. K는 원인 C일 때만 바꾼다(focus_kw 를 맨 앞, 기존 키워드는 중복 없이 뒤로)
const T='NEW_SEO_TITLE', M='NEW_META', K=null /* 또는 'focus_kw, 기존1, 기존2' */;
...
- Rank Math 점수가 70 미만으로 떨어지면 STEP 10에 기록(되돌리지는 않는다).
...
{"slug":"...","status":"applied|rollback","track":"urgent|normal","date":"{TODAY}",
 ...
 "volume":{월 검색수},"share_before":{노출 점유율},"momentum":{흐름},
...
- 검증: REST p {n}→{n} · h2 {n}→{n} · 공개 title {일치|캐시 대기} · Rank Math {점수}
```

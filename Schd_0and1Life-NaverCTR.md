...
🔴 [최우선 원칙 — 2026-09-26 사용자 지시, 다른 모든 규칙보다 앞선다]
이 루틴을 매일 돌리는 목적은 **CTR 개선** 그 자체다. 숏테일(대표 키워드)만이 아니라 **롱테일(TOP30에 안 잡히는 나머지 검색어) CTR 개선**도 같은 무게의 목적이다.
1. 글이 안 눌리는 **원인이 분명하면(STEP 7-0 "원인 확정")** 효과 판정 대기(WAIT)·재수정 간격·최소 수정 원칙을 기다리지 않고 **그날 바로 고친다.** 본문의 대부분을 다시 쓰는 한이 있어도 원인을 해결하는 수정을 한다(예: 2026-09-26 재산세 글 — 검색 의도는 "카드사별 무이자 목록"인데 글은 "이득 계산"이고 본문에 "카드사별 표는 의미 없다"는 문단까지 있었다 → 공식 출처 카드사 7곳 비교표를 첫 화면에 넣고 제목·메타·첫 문단을 목록형으로 교체).
2. 원인이 분명한 글 수정은 **새 대상 선정보다 먼저** 한다(STEP 5-0).
3. 원인이 분명하지 않을 때만 기존 방식(제목·메타·첫 문단 중심의 최소 수정 → 효과 판정 → 재수정)을 따른다.
4. 어떤 경우에도 지키는 것: 슬러그(URL)·status·카테고리·발행일 불변 · 사실은 본문 또는 공식 출처로 확인한 것만 · 이미지 삭제 금지 · 백업 후 반영 · 반영 직후 전수검사(8-5) · Rank Math 수정 전 −2 이내·70 이상.
...
7. **CTR 개선은 글 전체 기준으로 한다**(2026-09-24 사용자 지시) — 제목을 바꾸면 그 때문에 어긋나는 곳(Focus Keyword·부제목·이미지 대체 텍스트·키워드 밀도)도 같이 맞춰 **Rank Math 점수를 수정 전 수준으로 유지**한다. 원인이 분명하지 않으면 어긋나는 곳만 최소로 고치고, **원인이 분명하면 본문 대부분을 다시 쓰더라도 원인을 해결한다**(2026-09-26 사용자 지시 — 최우선 원칙)
8. **수정한 글은 반영 후 7일 동안 매일 일자별 노출·클릭·CTR 표로 모니터링**한다(`monitor/{slug}.md`, 매일 알림에 포함)
9. **매일 가장 먼저 "원인 확정 글"을 찾는다**(2026-09-26) — 반영 후 모니터링 중인 글 + 대기열 상위 글을 STEP 7-0 기준으로 진단해, 원인이 분명한 글을 그날 1순위로 고친다. 숏테일(TOP30 키워드)과 롱테일(문서 노출 − TOP30 키워드 노출)을 따로 본다
...
⚠️ [시간·에디터 충돌]
2026-09-26 확인: `0and1life auto image insert` 예약은 **05:30 KST** 로 옮겨져 이 루틴(06:30)보다 먼저 끝난다. 그래서 예전 "06:45~08:00 반영 금지" 창은 **없앤다** — 전면 수정처럼 오래 걸려도 끝까지 반영한다.
- 대신 에디터를 열 때마다 **같은 글을 다른 세션이 편집 중인지** 확인한다(8-3 ①의 `isPostLocked()`·`dirty`). 잠겨 있으면 저장하지 않고 60초 뒤 1회 재확인 → 그래도 잠겨 있으면 본문 REST 반영도 백업으로 원복하고 `에디터 잠김 — 보류` 기록(엔진 기록 안 함 → 다음 날 1순위).
- 07:00 KST 에 `KoreaPlug 구글 CTR 개선` 이 같은 Chrome을 쓴다. 이 루틴은 **자기 탭 그룹의 탭만** 쓰고, 다른 탭은 이동·닫기 하지 않는다.
...
⚠️ 변경 금지 항목(항상): 글의 status(publish/draft/private)·**슬러그(URL)**·카테고리·발행일·기존 이미지 삭제.
⚠️ 본문 수정 범위:
- **원인 미확정(기본 수정)**: 첫 문단·답 블록(STEP 7-5)·정합성 보정(STEP 7-6)만. H2 순서·FAQ 문구·ld+json 유지.
- **원인 확정(전면 수정, STEP 7-7)**: 원인을 해결하는 데 필요한 만큼 본문 대부분을 다시 써도 된다 — H2 추가·수정·순서 변경, 문단 재작성, 표·체크리스트 추가, 틀리거나 낡은 사실 교체, 검색 의도와 반대되는 문장 삭제. FAQ를 바꾸면 **화면의 FAQ 문구와 ld+json(FAQPage)을 함께** 바꾸고 질문 수를 일치시킨다.
...
FIX_MODE     = AUTO      # 원인 확정 글 전면 수정(2026-09-26 추가 · 최우선): AUTO = 그날 바로 반영 / SAMPLE = 수정안만
FIX_PER_DAY  = 1         # 하루 원인 확정 전면 수정 글 수(REEDIT·NEW 한도와 별도)
```
...
- ⚠️ `demand.py` 는 넘겨준 plan의 `demand_targets` 만 파일에 **덮어쓴다**(2026-09-26 실측: 2차 호출에서 앞 대상 수요가 사라짐). 4-1~4-3을 다시 해서 대상이 바뀌면, 앞 호출 대상과 새 대상을 **합친 plan 파일**을 만들어 한 번 더 조회한다.
...
0. **원인 확정 전면 수정 (최우선 — 2026-09-26)** — FIX_PER_DAY 편.
   - 진단 대상: ① `evaluations[]` 의 모든 글(WAIT·REEDIT·ROLLBACK·LIMIT 포함, HOLD·KEEP 제외) ② `queue[]` 상위 3편. 긴급(이벤트 임박) 글·월 검색수 큰 글부터.
   - 각 글에 STEP 6-2(상위10 조사)와 STEP 7-0(원인 확정 진단)을 한다. **"원인 확정"이 나온 첫 글**을 오늘 전면 수정(STEP 7-7) 대상으로 정한다. 판정 대기(WAIT) 중이어도, 반영한 지 하루여도 고친다.
   - 그 글이 `rollback[]` 에 있으면 되돌리지 않고 전면 수정으로 대신한다(되돌리면 같은 원인이 남는다).
   - 원인 확정 글이 없으면 진단 결과(글별 한 줄)만 log.md에 남기고 1번으로 간다.
   - 진단은 최대 4편, 상위10 조사 호출은 편당 2회까지.
...
⚠️ 같은 글이 전면 수정·되돌림·재수정·새 대상에 동시에 걸릴 수 없다(엔진이 잠금으로 막는다). 엔진 출력(`evaluations`·`queue`·`reedit`·`rollback`)에 없는 글은 손대지 않는다. 전면 수정은 엔진 판정을 기다리지 않는 유일한 예외다.
...
[7-0] 원인 확정 진단 (2026-09-26 추가 — STEP 5-0에서 매일 먼저 실행)
근거는 **데이터와 상위10 조사로 확인한 것만** 쓴다. 아래 증거 중 **2개 이상**이 맞으면 `원인 확정` 이다.
| # | 증거 | 확인 방법 |
|---|---|---|
| E1 | **숏테일 무클릭**: 대표 키워드 노출은 최근 2일 이상 늘었는데 그 키워드 클릭 증가가 0~1 | 스냅샷 `keywords` 의 그 키워드 노출·클릭 날짜별 증가분 |
| E2 | **롱테일 무클릭**: 문서 노출 − TOP30 연결 키워드 노출(롱테일) 증가분이 하루 100 이상인데 롱테일 CTR이 목표 CTR의 1/3 미만 | 스냅샷 `docs`·`keywords` 증가분으로 계산 |
| E3 | **의도 유형 불일치**: 상위10 중 6개 이상이 같은 답 유형(목록·비교표·날짜·금액·방법·공식 신청 페이지)인데 우리 제목·첫 화면 답은 다른 유형 | 6-2 상위10 제목·요약 |
| E4 | **필수 정보 누락**: 상위 정보글 3편이 공통으로 주는 정보(카드사별 조건·지역별 금액·마감일 등)가 우리 본문에 없음 | 6-3 벤치마크 + 우리 `content.raw` |
| E5 | **본문이 의도와 반대**: 검색자가 원하는 답을 부정·회피하는 문장("표는 의미 없다", "정답은 없다" 등)이나 결론이 없는 도입 | 우리 본문 |
| E6 | **사실 낡음·틀림**: 본문 수치·날짜·조건이 올해 공식 공지와 다르거나 지난 연도 기준 | 공식 출처 WebFetch(막히면 Claude in Chrome으로 공식 페이지 읽기) |
- 결과를 log.md에 `진단: {slug} — 원인 확정(E1·E3·E4) / 원인: {한 줄} / 해결: {한 줄}` 또는 `원인 미확정(E1만)` 으로 남긴다.
- 원인 확정이면 7-7(전면 수정), 아니면 기존 7-1~7-6(기본 수정)을 따른다.
- 숏테일과 롱테일 중 **어느 쪽이 클릭을 잃는지** 반드시 적는다. 롱테일이 크면 제목을 대표 키워드 하나로 좁히지 않고, 롱테일 검색어들이 공통으로 묻는 답(목록·비교·지역·조건)을 제목 뒷부분·H2·표로 받는다.
...
- ⛔ 금지: 본문·공식 출처에 없는 사실·수치 / 과장·낚시 / 키워드 반복 / 슬러그 변경. (원인 미확정 기본 수정에서는 기존 H2 변경·첫 문단·답 블록 외 본문 수정도 금지 — 원인 확정 전면 수정은 7-7을 따른다.)
...
[7-7] 원인 확정 전면 수정 (2026-09-26 추가 — 최우선)
목표: 7-0에서 찾은 **원인을 없애는 것**. 수정 범위는 원인이 요구하는 만큼이며, 본문 대부분을 다시 써도 된다.
- 처방 대응:
  | 원인 | 처방 |
  |---|---|
  | E3 의도 유형 불일치 | 제목·메타·첫 문단을 상위10의 답 유형(목록형 "n곳 비교"·마감형·금액형 등)으로 바꾸고, 그 답을 **목차 바로 뒤 첫 섹션**에 둔다 |
  | E4 필수 정보 누락 | 공식 출처로 확인한 비교표·목록·체크리스트를 새 섹션으로 넣는다(행 수 제한 없음, 확인 못 한 행은 만들지 않음) |
  | E5 의도와 반대 문장 | 해당 문장·문단을 새 답을 가리키는 문장으로 바꾸거나 지운다 |
  | E6 사실 낡음 | 공식 공지 값으로 모든 곳(본문·표·FAQ·ld+json·메타)을 한꺼번에 고친다 |
  | E1·E2 무클릭 | 위 처방 + 롱테일 검색어 묶음이 묻는 것을 H2·표로 받는다(연관 키워드는 `demand/{TODAY}.json` 의 `related[]`, `naver_volume.py --related`) |
- 절차: ① 공식 출처 확인(WebFetch → 막히면 Claude in Chrome `get_page_text`, 확인 날짜 기록) ② 새 `content.raw` 전체를 python으로 만든다(기존 HTML 스타일·이미지·내부 링크·면책/글쓴이 박스 유지) ③ 변경 전후 **태그 수 차이를 코드로 계산**해 8-2 기대값으로 쓴다 ④ SEO 제목·메타·FK는 7-3·7-6 규칙대로.
- 유지: 슬러그·status·카테고리·발행일·모든 이미지(`<img` 수 감소 금지)·내부 링크 수(감소 금지)·글쓴이/한계 박스·본문 1,500자 이상·문체.
- FAQ를 바꾸면 화면 FAQ 질문 수 = ld+json `Question` 수, ld+json 파싱 성공.
- 기록: 엔진 `record` 의 `cause` 는 `"확정:E3+E4+E5"` 처럼, `ctr_before` 는 **그 글의 최초 수정 전 CTR**(v1 before)을 쓴다 — 여러 번 고쳐도 원래 기준과 비교해야 하기 때문이다. `note` 에 원인·해결 한 줄과 공식 출처 URL·확인일.

---

## STEP 8 — 반영 (FIX·URGENT·NORMAL 모두 AUTO)
...
2) **전면 수정(7-7)**이면 7-7 ②에서 만든 새 `content.raw` 전체를 POST한다(기대 태그 수 = 새 content를 코드로 센 값). **기본 수정**이면 python으로 `content.raw` 의 **style 없는 첫 `<p>…</p>`** 하나만 새 문단으로 바꾸고(모든 `<p>` 에 style이 있는 글은 첫 `<p>` 의 이미지 뒤 글만 바꾼다 — 2026-09-26 시그니엘 글), 7-6 보정(H2 1개·alt 1개·라벨 1곳 — 각각 정확히 1곳 일치를 assert)을 같이 적용하고, 답 블록이 있으면 7-5 위치에 넣어 `POST /wp-json/wp/v2/posts/{id}` `{"content": ...}`. 원래 문단에 기대한 문구가 있는지, 삽입 기준 표시가 정확히 1개인지 assert 후 교체한다.
3) 8초 후 재조회: `status=publish` 유지 · `<p`·`<h2`·`<table`·`<img` 개수 = POST한 content를 코드로 센 값 · 새 문장 포함 · 옛 문장 없음. 하나라도 어긋나면 백업 raw로 즉시 되돌리고 `본문 반영 실패 — 원복` 기록, 8-3은 건너뛴다.
...
({ title: s.getSerpTitle(), desc: s.getSerpDescription(), fk: s.getKeywords(), score: s.getAnalysisScore?.(),
   p: (ed.getEditedPostContent().match(/<p/g)||[]).length, dirty: ed.isEditedPostDirty(),
   locked: ed.isPostLocked?.() })   // locked=true 이거나 dirty=true 면 저장하지 않는다(다른 세션 편집 중)
```
```javascript
...
| 구조 | `<p`·`<h2`·`<figure`·`<img`·`<table`·`<a `·`wp:freeform` 개수 | POST한 content를 코드로 센 값과 정확히 같음 · 백업 대비 `<img`·내부 링크 감소 없음 · freeform 마커 수 유지 |
| 태그 짝 | div·ul·ol·table·figure·blockquote·p·h2·strong·a 여닫이 수 | 모두 같음(`<ul[\s>]` 정규식으로 셀 것) |
| 스키마 | ld+json 개수와 JSON 파싱 · FAQ | 개수 유지 · 전부 파싱 성공 · 화면 FAQ 질문 수 = FAQPage `Question` 수 |
...
| 공개 페이지 | `?nc={epoch}` 의 `<title>`·description·robots·H1·H2 목록·목차 | 새 제목·메타 반영 · index 유지 · H2 목록이 POST한 content와 같음 · 목차에 바뀐 H2 반영 |
| 리소스 | 본문 이미지(0and1life.com) HTTP · 내부 링크 | 전부 200 · 내부 링크 수 유지(외부 기관 링크는 클라우드에서 000일 수 있음 — `확인 불가`로만 기록) |
| 품질 | 첫 문단 길이(기본 수정만 기존 ±30%) · 키워드 반복(같은 문단 2회 이상 금지) · 본문 글자수 · 사실 | 기준 안 · 1,500자 이상 · 새로 쓴 수치·날짜는 전부 본문 원래 값 또는 공식 출처(확인일 기록)와 일치 |
...
{"slug":"...","status":"applied|rollback","track":"urgent|normal","date":"{TODAY}",
 "keyword":"{focus_kw}","ctr_before":{판정 시점 CTR — 전면 수정은 최초 수정 전 CTR},"impr_before":{노출},"published":"{발행일}","cause":"A|B|C|확정:E…",
...
```
- 반영 후 7일 안의 모든 글에 대해 `monitor/{slug}.md` 와 `monitor/_summary.md` 를 새로 쓴다(2026-09-25 개편): **같은 글을 D+0(반영일 기준)~D+7 일자별 행으로** 보여 주고, 날마다 일 노출·클릭·CTR · 반영 전 CTR 대비 증감 · **일 판정** · 반영 후 누적 노출·클릭·CTR · **누적 판정**을 적는다. 아직 오지 않은 날은 `대기` 행으로 미리 둔다.
- 판정(반영 전 CTR 기준): ▲개선(×1.3 또는 +0.5%p 이상) · △소폭 상승 · ▽소폭 하락 · ▼악화(×0.7 미만) · 노출 100 미만은 보류. **좋아졌는지는 누적 판정으로 말한다**(하루 값은 클릭 1~2번에 흔들린다). 글 맨 위에 `지금 판정` 한 줄과 최근 3일 일 CTR 흐름(오르는 중·내리는 중·오르내림)을 붙인다.
- 발행 30일 이내 글은 정확, 그보다 오래된 글은 근사(표에 표시). 중복 CSV 날은 빈 날로 표시된다.
- 7일이 지나면 `monitor_done` 으로 닫히고 효과 판정(evaluate)으로 넘어간다.

[9-3] `log.md` 맨 위에 추가
```
## {TODAY} · {v1 반영 | v2 재수정 | 전면 수정(원인 확정) | 되돌림} · {slug}
- 진단(7-0): {원인 확정(E…) | 미확정} — 숏테일 {키워드 노출 +n·클릭 +n} / 롱테일 {노출 +n·클릭 +n·CTR x%} — 원인 {한 줄} → 해결 {한 줄}
...
- PushNotification 한 번으로 보낸다. `<routine_summary>` 의 **첫 문장**(휴대폰 배너)은 한 줄 요약, **그 뒤(이메일 본문)에 아래 형식 전체와 `monitor/_summary.md` 의 표를 요약하지 말고 그대로** 넣는다(2026-09-26 사용자 지적: 표를 빼고 한 줄로 줄여 보낸 사고).
- 루틴의 마지막 답변에도 같은 표(한눈 표 + 글별 일자별 표)를 그대로 붙인다.

```
🔎 네이버 CTR 루틴 {TODAY}
- 데이터: {스냅샷 날짜} (stale {n}일{· 중복 CSV {날짜}}) · 목표 CTR {x}% · 수요 {조회됨 | 트렌드만 | 실패}
- 7일 트래킹: `monitor/_summary.md` 를 **그대로** 붙인다 — ① 한눈 표(글 · 반영일 · 경과 · 반영 전 CTR · 반영 후 누적 노출·클릭·CTR · 기준 대비 · 지금 판정 · 일 CTR 흐름 · Rank Math) ② **글마다 D+0~D+7 일자별 표**(일 노출·클릭·CTR · 일 판정 · 누적 CTR · 누적 판정). 알림에는 글마다 `{slug}: 지금 판정 · 누적 CTR {전}→{후}` 한 줄과 새로 채워진 날의 행을 적는다. 노출은 수요에 흔들리므로 효과는 CTR로 먼저 판단한다
- 원인 확정 진단: {slug — 확정(E…) → 전면 수정 | 미확정(E…)} …
...
- 오늘 작업: {전면 수정·재수정·되돌림·새 반영} — {slug} — 원인 {A|B|C|확정:E…} — {focus_kw} 월 {n}회 · 점유율 {x}%
  제목: {전} → {후}{ · 답 블록 추가}
...
- **예외(2026-09-26)**: 원인 확정(STEP 7-0) 글은 위 판정 시점·재수정 간격을 기다리지 않고 전면 수정한다. 전면 수정 뒤에는 새 버전으로 다시 7일 모니터링하고, 비교 기준은 최초 수정 전 CTR이다.
- 참고 지표: `share`(노출 점유율)가 오르면 순위가 오른 것, CTR이 오르면 제목이 먹힌 것이다. 둘을 따로 보고한다.

## 색인 영향 메모

(2026-09-26 개정) 원인 확정 글은 본문을 크게 바꿀 수 있다. URL·canonical·발행일은 그대로라 같은 문서의 갱신으로 처리되지만, 대량 교체는 순위가 며칠 흔들릴 수 있다. 사용자 판단: **원인이 분명하면 CTR 개선을 우선한다.** 대신 같은 글의 전면 수정은 원인이 새로 확정될 때만(같은 원인으로 연속 전면 수정 금지) 한다.
...
| 에디터에서 같은 글이 잠김(`isPostLocked`)·dirty | 저장 안 함 → 60초 후 1회 재확인 → 그래도면 본문 원복·`에디터 잠김 — 보류` |
| 원인 확정인데 공식 출처 확인 불가 | 확인된 원인(E3·E5 등)만 고치고 확인 못 한 표·수치는 넣지 않음, STEP 10에 `공식 출처 확인 필요` |
| 전면 수정 후 `<img`·내부 링크 감소, FAQ 수 불일치 | 백업 raw·이전 SEO 값으로 원복, 엔진 기록 안 함 |
